...
from oxo_logic_exercise1 import Game
...
def evNew():
		...
		game2cells(ex1.board)

def evResume ():     
      ex1.status['text'] = "Playing game"
      game = ex1.restoreGame()
      game2cells(game)

def evSave():
      ex1.board = cells2game()
      ex1.saveGame(game)
...
def evClick(row,col): 
	...
	ex1.board = cells2game()
	index = (3*row) + col
	result = ex1.userMove(index)
	game2cells(ex1.board)
	
	if not result:
		result = ex1.computerMove()
		game2cells(ex1.board)
	if result == "D":
